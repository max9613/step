...
public final class FindMeetingQueryTest {
  ...
  @Test
  public void everyAttendeeIsConsideredPlusOptionalFullDay() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times.
    // includes an optional attendee with an allday event.
    //
    // Events  :       |--A--|     |--B--|
    // Day     : |-----------------------------|
    // Options : |--1--|     |--2--|     |--3--|
  ...
  @Test
  public void everyAttendeeIsConsideredPlusOptionalBeginningOfDay() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times.
    // includes an optional attendee with a beginning of the day event.
    //
    // Events  :       |--A--|     |--B--|
    // Day     : |-----------------------------|
    // Options : |--1--|     |--2--|     |--3--|
  ...
  @Test
  public void justEnoughRoomPlusOptional() {
    // Have one person, but make it so that there is just enough room at one point in the day to
    // have the meeting, with an optional person.
    //
    // Events  : |--A--|     |----A----|
    // Day     : |---------------------|
    // Options :       |-----|
  ...
  @Test
  public void optionalsWithGaps() {
    // There are gaps between schedueles so there are multiple valid options.
    // have the meeting.
    //
    // Events  : |A|  |B|  |A|     |-B-|
    // Day     : |---------------------|
    ...
    Collection<Event> events = Arrays.asList(
        new Event("Event 1", TimeRange.fromStartEnd(TimeRange.START_OF_DAY, TIME_0830AM, false),
            Arrays.asList(PERSON_A)),
        new Event("Event 2", TimeRange.fromStartEnd(TIME_0900AM, TimeRange.getTimeInMinutes(9, 45), false),
            Arrays.asList(PERSON_B)),
        new Event("Event 3", TimeRange.fromStartEnd(TimeRange.getTimeInMinutes(10, 30), TimeRange.getTimeInMinutes(11, 0), false),
            Arrays.asList(PERSON_A)),
        new Event("Event 4", TimeRange.fromStartEnd(TimeRange.getTimeInMinutes(15, 30), TimeRange.getTimeInMinutes(16, 0), false),
            Arrays.asList(PERSON_A)));
    ...
    Collection<TimeRange> expected =
        Arrays.asList(TimeRange.fromStartDuration(TIME_0830AM, DURATION_30_MINUTES),
        TimeRange.fromStartDuration(TimeRange.getTimeInMinutes(9, 45), 45),
        TimeRange.fromStartDuration(TimeRange.getTimeInMinutes(11, 00), TimeRange.getTimeInMinutes(4, 30)),
        TimeRange.fromStartEnd(TimeRange.getTimeInMinutes(16, 0), TimeRange.END_OF_DAY, true));

    Assert.assertEquals(expected, actual);
  }

  @Test
  public void optionalsWithoutGaps() {
    // There is no gap between schedueles so there are no valid options.
    // have the meeting.
    //
    // Events  : |----A---||-----B-----|
    // Day     : |---------------------|
